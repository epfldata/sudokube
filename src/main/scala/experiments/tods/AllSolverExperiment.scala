...
class AllSolverExperiment(ename2: String)(implicit timestampedFolder: String, numIters: Int) extends Experiment(ename2, "all-solvers", "tods23") {
  var runID = 0
  ...
  def runNaive(dc: DataCube, query: IndexedSeq[Int]) = {
    Profiler.resetAll()
    val prepared = Profiler("PrepareNaive") {
      dc.index.prepareNaive(query)
    }
    val result = Profiler("FetchNaive") {
      dc.fetch(prepared).map(p => p.sm)
    }
