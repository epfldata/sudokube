...
class ErrorAnalysis(ename2: String = "")(implicit timestampFolder: String) extends Experiment("moment-error", ename2, "error-expt") {
  ...
  override def run(dc: DataCube, dcname: String, qu: IndexedSeq[Int], trueResult: Array[Double], output: Boolean = true, qname: String = "", sliceValues: IndexedSeq[Int] = Vector()): Unit = {
    val q = qu.sorted
    ...
    val transformer = Moment1TransformerDouble()
    ...
    val trueMus = transformer.getCoMoments(trueResult, solver.pmMap)
    val predMus = transformer.getCoMoments(solver.solution, solver.pmMap)
    val ipfMus = transformer.getCoMoments(ipf.solution, solver.pmMap)
