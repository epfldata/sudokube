...
object DemoTxt {
  ...
  def vanillaIPFSolver(): Unit = { // Bad case for IPF — 2000+ iterations
    ...
    solver.add(List(0, 1), Array(1001, 2000, 2000, 1001).map(_.toDouble))
    solver.add(List(1, 2), Array(1001, 2000, 2000, 1001).map(_.toDouble))
    solver.add(List(0, 2), Array(1001, 2000, 2000, 1001).map(_.toDouble))
    val result = solver.solve()
